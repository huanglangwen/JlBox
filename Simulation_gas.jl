...
reset_timer!(to); @timeit to "CVODE_BDF TFQMR" begin sol=solve(prob,CVODE_BDF(linear_solver=:TFQMR),dense=false); end; show(to);println()
reset_timer!(to); @timeit to "CVODE_BDF BCG" begin sol=solve(prob,CVODE_BDF(linear_solver=:BCG),dense=false); end; show(to);println()
...
reset_timer!(to); @timeit to "KenCarp4" begin sol=solve(prob,KenCarp4(autodiff=false),dense=false); end; show(to);println()
