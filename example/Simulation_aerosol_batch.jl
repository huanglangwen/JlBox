...
function configure_aerosol(io::Base.IO)
...
function configure_aerosol_solver_dense(solver, diff_method)
    sparse=false
    reltol=1e-4
    abstol=1.0e-2
    dtinit=1e-6
...
function simulate_aerosol(solver, diff_method)
    open("aerosol_batch.log", "a") do io
        config = configure_aerosol(io)
        solverconfig = configure_aerosol_solver_dense(solver, diff_method)
        sol, reactants2ind, param_dict = JlBox.run_simulation(config, solverconfig)
    end
end

open("aerosol_batch.log", "w") do io
    write(io, "")
end
for solver in [OrdinaryDiffEq.TRBDF2(), Sundials.CVODE_BDF()]
    for diff_method in ["coarse_seeding","fine_seeding","coarse_analytical","fine_analytical","finite"]
        println("Simulating APINENE mixed phase with $diff_method and $solver")
        simulate_aerosol(solver, diff_method)
    end
end